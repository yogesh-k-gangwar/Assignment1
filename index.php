...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  $fname = test_input($_POST["fname"]);                       //first name validation
  // check if name only contains letters and whitespace
  if(!preg_match("/^[a-zA-Z ]*$/",$fname)) {
    $count++;
    $fnameErr = "Only letters and white space allowed";
  }
  $lname=test_input($_POST["lname"]);                        //last name validation
  // check if name only contains letters and whitespace
  ...
  $contact = test_input($_POST["cont"]);                  //contact validation
  ...
  $pwd=test_input($_POST["pswd"]);                      //password validation
  ...
  if($count>0){     //checking for errors
    $_SERVER['PHP_SELF'];
  }
  else{
